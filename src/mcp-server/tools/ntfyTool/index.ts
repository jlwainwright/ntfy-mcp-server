...
import { SendNtfyToolInput, SendNtfyToolInputSchema } from "./types.js";
...
export const registerNtfyTool = async (server: McpServer): Promise<void> => {
  ...
  try {
    ...
    server.tool(
      "send_ntfy",
      `Send notifications to the user's devices using ntfy.sh service with support for titles, priorities, tags, attachments, and actions. Use this tool to externally notify the user of something important. ${defaultTopicInfo}.`,
